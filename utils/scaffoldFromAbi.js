...
schema:
  file: ./schema.graphql
dataSources:
  ${dataSources.map((dataSource) => `- ${dataSource}`).join('\n  ')}
  `,
    {
      parser: 'yaml',
  ...
  directories.forEach((path) => {
    ...
    if (stat.isFile(path)) {
      if (
        ...
        !path.toLowerCase().includes('mock') &&
        !path.toLowerCase().includes('dummy')
      ) {
        paths.push(path)
      }
    }
...
const startScaffoldAll = async () => {
  ...
  const promises = paths.map(async (filePath) => {
    try {
      ...
      console.log(`updating mustache json file for ${contractName}`)
      updateMustacheConfigFile({
        contractName,
        network,
        address,
      })
...
const startScaffoldAbi = async (filepath, address, blockNumber, isGenerateMapping, isGenerateSchema) => {
  ...
  console.log(`adding ${contractName} entities to schema.graphql`)
...
const run = async () => {
  program
    ...
    .requiredOption(
      ...
      'if scaffoldAll path to contract abi directory, if not path to contract abi file',
    )
